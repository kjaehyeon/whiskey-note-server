...
@DisplayName("[통합테스트] CONTROLLER - AUTH")
...
class AuthControllerTest {
    ...
    @DisplayName("[AUTH][POST] 관리자 회원가입 - 정상입력하면 회원정보 추가하고 HTTP 200 리턴")
    @Test
    void givenValidAdminInfo_whenSignUp_thenCreateAdminAndReturns() throws Exception{
        //Given
        SignUpRequest signUpRequest = SignUpRequest.of(
                "admin1",
                "password1",
    ...
    @DisplayName("[AUTH][POST] 관리자 회원가입 - 유효하지 않은 정보 받으면, HTTP 400 리턴")
    @Test
    void givenInvalidAdminInfo_whenSignUp_thenCreateAdminAndReturns() throws Exception{
        //Given
        SignUpRequest signUpRequest = SignUpRequest.of(
                "",
                "password1",
    ...
    @DisplayName("[AUTH][POST] 관리자 회원가입 - 관리자 정보 중복 될때 USER_ALREADY_EXISTS 예외 반환")
    @Test
    void givenDuplicatedAdminInfo_whenSignUp_thenRetrunException() throws Exception{
        //Given
        createNormalAdmin("admin1", "password1");
        SignUpRequest signUpRequest = SignUpRequest.of(
                "admin1",
                "password1",
                "admin1@email.com"
    ...
    @DisplayName("[AUTH][POST] 로그인 - 아이디가 없는 경우")
    @Test
    ...
    //회원 비밀번호 틀렸을때
    @DisplayName("[AUTH][POST] 로그인 - 비밀번호를 틀린경우")
    @Test
    ...
    private void createNormalAdmin(String username, String password){
        userRepository.save(User.builder()
                .username(username)
                .password(passwordEncoder.encode(password))
                .email(username + "@email.com")
